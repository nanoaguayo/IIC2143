...
//Hacemos otro para la interfaz
ArrayList<RamoAvanceFX> GetHistorialFX(Alumno a){
	String path = "data/Alumnos/";
	String num=String.valueOf(a.numero_alumno);
	path+=num;
	path+="/Historial.txt";
	...
	ArrayList<RamoAvanceFX> lista = new ArrayList<RamoAvanceFX>();
	for(int i=0;i<resultado.length;i++){
		int largo = resultado[i].Ramos.length;
		String periodo=resultado[i].Periodo;
		for(int j=0;j<largo;j++){
			Ramo AUX = resultado[i].Ramos[j];
			RamoAvanceFX AUX2 = new RamoAvanceFX(AUX.Nombre,AUX.Sigla,AUX.Nota);
			AUX2.setPeriodo(periodo);
			lista.add(AUX2);
		}
	}
	return lista;
